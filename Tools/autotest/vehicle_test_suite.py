...
#            print("t=%s" % str(t))
    ...
    def assert_distance_sensor_distance(self, distance, epsilon=0.5):
        m = self.assert_receive_message('DISTANCE_SENSOR')

        if abs(m.distance_cm * 0.01 - distance) > epsilon:
            raise NotAchievedException("Unexpected distance; want={distance} got={m.distance_cm*0.01}")
