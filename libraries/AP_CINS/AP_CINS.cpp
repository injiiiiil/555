...
void AP_CINS::update_imu(const Vector3F &gyro_rads, const Vector3F &accel_mss, const ftype dt)
{
    ...
    const Vector3F& left_delay_vel = -gravity_vector*current_lag;
    const Vector3F& left_delay_pos = -gravity_vector*0.5*current_lag*current_lag;
...
void AP_CINS::update_vector_measurement_cts(const Vector3F &measurement, const Vector3F& reference, const Vector2F &ref_base, const ftype& gain_R, const ftype& gain_V, const ftype& gain_gyr_bias, const ftype& gain_acc_bias, const ftype dt) {
    ...
    const GL2& S_Gamma = -0.5 * (gain_V) * heapVars.ZInv.A() * CCT * heapVars.ZInv.A().transposed();
    ...
    heapVars.K21 = I3 * gains_AInvC.x * (gain_R + gain_V) * dt;
    heapVars.K31 = I3 * gains_AInvC.y * (gain_R + gain_V) * dt;
    ...
    heapVars.A21 = - Matrix3F::skew_symmetric(GammaInv.W1());
    heapVars.A22 = I3*GammaInv.A().a11();
    heapVars.A23 = I3*GammaInv.A().a21();
    heapVars.A31 = - Matrix3F::skew_symmetric(GammaInv.W2());
    heapVars.A32 = I3*GammaInv.A().a12();
    heapVars.A33 = I3*GammaInv.A().a22();
    ...
    heapVars.Delta_SIM23 = SIM23(Matrix3F::from_angular_velocity(Omega_Delta*dt), W_Delta1*dt, W_Delta2*dt, GL2::identity());
